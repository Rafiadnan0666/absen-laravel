      ...
      <div class="p-6 pb-0 mb-0 bg-white border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          ...
          <div class="flex items-center gap-2 w-full sm:w-auto flex-wrap">
            <form method="GET" action="{{ route('hr.leaves.index') }}" class="flex items-center gap-2 flex-wrap">
              ...
              <select name="type" class="px-3 py-2 text-xs border border-slate-200 rounded-lg focus:border-purple-400 outline-none bg-white">
              ...
              <a href="{{ route('hr.leaves.export', request()->query()) }}" class="inline-block px-4 py-2 mb-0 text-xs font-bold text-right uppercase align-middle transition-all border-0 rounded-lg cursor-pointer hover:scale-102 active:opacity-85 bg-gradient-to-tl from-green-600 to-lime-400 text-white whitespace-nowrap">
          ...
          <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                  <span class="px-2 py-1 text-xs rounded-2xl inline-block whitespace-nowrap text-center text-white bg-gradient-to-tl from-blue-600 to-cyan-400">{{ strtoupper($item->tipe_cuti) }}</span>
                </td>
                <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                  <p class="mb-0 leading-normal text-xs text-slate-500">{{ \Carbon\Carbon::parse($item->tanggal_mulai)->format('d M') }} - {{ \Carbon\Carbon::parse($item->tanggal_selesai)->format('d M Y') }}</p>
                </td>
                <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                  <p class="mb-0 leading-normal text-xs text-slate-500 max-w-[120px] truncate">{{ Str::limit($item->alasan, 30) }}</p>
                </td>
                <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
                  <span class="px-3 py-1 text-xs rounded-2xl inline-block whitespace-nowrap text-center text-white font-bold
                    @if($item->status_pengajuan == 'approved') bg-gradient-to-tl from-green-600 to-lime-400
                    @elseif($item->status_pengajuan == 'pending') bg-gradient-to-tl from-purple-600 to-pink-500
              ...
              @empty
              <tr>
                <td colspan="6" class="p-4 text-center text-slate-400">No leave requests</td>
              </tr>
              @endforelse
            </tbody>
          </table>
