  ...
  <div class="flex-none w-full max-w-full p-3">
    <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
      ...
      <div class="flex-auto p-6">
        <div class="mb-4">
          <label class="inline-block mb-2 text-sm font-bold text-slate-700">Title</label>
          <p class="text-sm text-slate-500">{{ $announcement->judul }}</p>
        </div>
        <div class="mb-4">
        ...
        <div class="mb-4">
          ...
          <p class="text-sm text-slate-500">{{ $announcement->creator->name ?? 'N/A' }}</p>
        </div>
        <div class="mb-4">
          <label class="inline-block mb-2 text-sm font-bold text-slate-700">Created At</label>
          <p class="text-sm text-slate-500">{{ $announcement->created_at->format('d M Y H:i') }}</p>
        </div>
        <div class="flex justify-end mt-6">
          <a href="{{ route('admin.announcements.edit', $announcement) }}" class="inline-block px-6 py-3 mb-0 text-xs font-bold text-right uppercase align-middle transition-all border-0 rounded-lg cursor-pointer hover:scale-102 active:opacity-85 leading-pro ease-soft-in tracking-tight-soft bg-150 bg-x-25 bg-gradient-to-tl from-blue-600 to-cyan-400 text-white mr-2">
