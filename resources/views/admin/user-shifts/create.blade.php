  ...
  <div class="neo-card">
    ...
    <form action="{{ route('admin.user-shifts.store') }}" method="POST">
      ...
      <div class="neo-form-group">
        ...
        <input type="date" id="tanggal_shift" name="tanggal_shift" value="{{ old('tanggal_shift') }}" required class="neo-input">
        @error('tanggal_shift')
          <p class="text-neo-red text-sm mt-1 font-bold">{{ $message }}</p>
        @enderror
      </div>
      <div class="flex justify-end">
        ...
        <button type="submit" class="neo-btn-primary">ASSIGN</button>
      </div>
    </form>
  </div>
</div>
