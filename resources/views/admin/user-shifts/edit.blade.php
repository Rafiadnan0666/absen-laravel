...
<div class="space-y-6">
  <div class="neo-card">
    <div class="mb-6 border-b-3 border-black pb-4">
      <h6 class="neo-section-title">EDIT USER SHIFT</h6>
    </div>
      ...
      <div class="neo-form-group">
        ...
        <select id="shift_id" name="shift_id" required class="neo-select">
          ...
          @foreach($shifts as $shift)
            <option value="{{ $shift->id }}" {{ old('shift_id', $userShift->shift_id) == $shift->id ? 'selected' : '' }}>{{ $shift->nama_shift }} ({{ $shift->jam_masuk }} - {{ $shift->jam_pulang }})</option>
          @endforeach
        </select>
        @error('shift_id')
          <p class="text-neo-red text-sm mt-1 font-bold">{{ $message }}</p>
