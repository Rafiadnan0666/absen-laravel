    ...
    <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border">
      ...
      <div class="flex-auto p-6">
        <div class="mb-4">
          ...
          <p class="text-sm text-slate-500">{{ $overtimeRule->id }}</p>
        </div>
        <div class="mb-4">
          <label class="inline-block mb-2 text-sm font-bold text-slate-700">Minimum Hours</label>
          <p class="text-sm text-slate-500">{{ $overtimeRule->minimal_jam }} hours</p>
        </div>
        <div class="mb-4">
          <label class="inline-block mb-2 text-sm font-bold text-slate-700">Multiplier</label>
          <span class="bg-gradient-to-tl from-green-600 to-lime-400 px-2 py-1 text-xs rounded-2xl inline-block whitespace-nowrap text-center text-white">{{ $overtimeRule->multiplier }}x</span>
        </div>
        <div class="flex justify-end mt-6">
          <a href="{{ route('admin.overtime-rules.edit', $overtimeRule) }}" class="inline-block px-6 py-3 mb-0 text-xs font-bold text-right uppercase align-middle transition-all border-0 rounded-lg cursor-pointer hover:scale-102 active:opacity-85 leading-pro ease-soft-in tracking-tight-soft bg-150 bg-x-25 bg-gradient-to-tl from-blue-600 to-cyan-400 text-white mr-2">
