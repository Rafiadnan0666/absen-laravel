    ...
    <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-soft-xl rounded-2xl bg-clip-border" x-data="tableFilter({ search: '', filterStatus: '', dateFrom: '', dateTo: '', amountMin: '', amountMax: '' })" x-init="init()">
      ...
      <div class="flex-auto p-6 px-0 pt-0 pb-2 relative">
        ...
        <div class="flex items-center gap-2 px-4 pt-2" id="bulk-actions" style="display: none;">
          ...
          <button type="button" onclick="document.getElementById('bulk-form').action='{{ route('admin.reimbursements.bulkApprove') }}'; document.getElementById('bulk-form').submit();" class="px-3 py-1 text-xs font-bold uppercase text-white rounded-lg bg-gradient-to-tl from-green-600 to-lime-400">Approve Selected</button>
          <button type="button" onclick="document.getElementById('bulk-form').action='{{ route('admin.reimbursements.bulkReject') }}'; document.getElementById('bulk-form').submit();" class="px-3 py-1 text-xs font-bold uppercase text-white rounded-lg bg-gradient-to-tl from-red-600 to-rose-400">Reject Selected</button>
        </div>
      </div>
    </div>
  </div>
