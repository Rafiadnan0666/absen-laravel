...
@section('title', 'Create Reimbursement - Admin')
  ...
  <div class="neo-card">
    ...
    <form action="{{ route('admin.reimbursements.store') }}" method="POST">
      ...
      <div class="mb-4">
        ...
        <input type="number" step="0.01" id="jumlah" name="jumlah" value="{{ old('jumlah') }}" class="neo-input" placeholder="ENTER AMOUNT" required>
        @error('jumlah')
          <p class="text-neo-red text-sm mt-1 font-bold">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="mb-4">
        ...
        <textarea id="deskripsi" name="deskripsi" rows="4" class="neo-input" placeholder="ENTER DESCRIPTION" required>{{ old('deskripsi') }}</textarea>
        @error('deskripsi')
          <p class="text-neo-red text-sm mt-1 font-bold">{{ $message }}</p>
        @enderror
      </div>
